...
export default function FileUploadField({ field, value, onChange, sizeClass }) {
  ...
  const isVideo = value && typeof value === 'object' && value.type && value.type.startsWith('video/');
  ...
  const handleFileChange = (e) => {
    ...
    if (file.size > maxSize) {
      alert(`El archivo es demasiado grande. Máximo ${file.type.startsWith('video/') ? '50MB para videos' : '10MB para imágenes'}.`);
      return;
    }
      ...
      <label className={`relative flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors overflow-hidden ${sizeClass}`}>
        {previewUrl ? (
          isVideo ? (
            ...
            <img src={previewUrl} alt="Vista previa" loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
          )
        ) : (
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p className={`text-gray-500 text-center ${sizeClass === 'text-xs' ? 'text-[8px]' : 'text-xs'}`}>PNG, JPG, MP4, MOV (Imágenes: 10MB, Videos: 50MB)</p>
          </div>
        )}
        <input
          type="file"
      ...
      {previewUrl && (
        <div className="flex gap-2 mt-2">
          <label className={`inline-flex items-center justify-center px-3 py-1.5 rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 cursor-pointer transition ${sizeClass === 'text-xs' ? 'text-[10px]' : 'text-sm'}`}>
            {isVideo ? 'Cambiar video' : 'Cambiar imagen'}
            <input
              type="file"
              className="hidden"
              accept="image/*,video/*"
